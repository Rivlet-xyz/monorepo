...
**A liquidity-aware collateral risk monitor for Ethereum mainnet lending protocols.**
...
This is an API-only submission for ETHOnline 2026. There is no dashboard; the
JSON API, the natural-language `/ask` endpoint, and the Foundry contract stub
are the deliverables. Every number is fetched live from The Graph Network at
request time or on a five-minute refresh. There is no mocked, cached, or fixture
data on any code path.
...
Other scripts: `bun run --cwd server probe:collect` (market counts and WETH pools),
`bun run --cwd server probe:depth <token>` (depth and pump cost for one token),
`bun test:server`, `bun typecheck`, `bun contracts:test`.
...
the numbers were computed from) and `x-shoalfi-refreshed-at`, so a client can
show exactly how live the data is.
...
Every file in this repository except the vendored `contracts/lib/forge-std` was
written with Claude Code (Claude Fable 5.1) working from a written
specification, with the tick-walk math, subgraph field names and Anthropic API
shapes verified against primary sources during implementation. Human review
covered the spec, the design decisions recorded in the commit history, and the
acceptance checks (probe output, WETH depth sanity check, unit and Foundry
tests). No other AI tools were used.
...
This repository was created on 2026-09-06 for a different idea (Rivlet) that was
abandoned the same week; that scaffold was removed in the first shoalfi commit
and all shoalfi work follows it.
